...
*)
   ...
   *)
    ...
    let ``08 An identifier pattern will match anything`` () =
        let x = 3 // replace with an integer
        let y = "hi" // replace with a string
        let z = fun e -> e // replace with anything else!
        x |> should be ofType<int>
        y |> should be ofType<string>

    [<Test>]
    let ``09 A wildcard pattern will match anything`` () =
        let _ = 3 // replace with an integer
        let _ = "sadgags"  // replace with a string
        let _ =  fun q -> q// replace with anything else!
        ()
